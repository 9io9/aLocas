...
Error fsalloc_init(FsAllocator* allocator, int chunk_size, int pmin, int pmax) {
    if (allocator == NULL || chunk_size <= 0) {
        return ERROR("allocator == NULL or chunk_size <= 0", ECODE(ALOCAS, FSA, ARGV));
    }
    ...
    if (allocator->page_list == NULL) {
        ...
        return ERROR("alloc_page return NULL, lack of memory", ECODE(ALOCAS, FSA, MEM));
    }
...
Result fsalloc(FsAllocator* allocator) {
    if (allocator == NULL) {
        return RESULT_FAIL("allocator == NULL", ECODE(ALOCAS, FSA, ARGV));
    }

    if (allocator->cur_page == NULL) {
        return RESULT_FAIL("allocator's page is NULL, allocator not init or init failed", ECODE(ALOCAS, FSA, INIT));
    }
    ...
    if (allocator->cur_page->slots_free == 0) {
        if (allocator->pnum == allocator->pmax) {
            return RESULT_FAIL("reach pmax, no more memory should be allocated", ECODE(ALOCAS, FSA, PMAX));
        }

        allocator->cur_page->next_page = alloc_page();

        if (allocator->cur_page->next_page == NULL) {
            return RESULT_FAIL("alloc_page return NULL, lack of memory", ECODE(ALOCAS, FSA, MEM));
        }
...
Error fsfree(FsAllocator* allocator, void** e) {
    if (e == NULL || *e == NULL || allocator == NULL) {
        return ERROR("e == NULL or *e == NULL or allocator == NULL", ECODE(ALOCAS, FSA, ARGV));
    }

    FsFreeEntry* ent = *e - sizeof(FsFreeEntry);

    if (!check(ent->check)) {
        return ERROR("checksum failed, not allocated from fsalloc", ECODE(ALOCAS, FSA, SCK));
    }
...
Error fsalloc_destroy(FsAllocator* allocator) {
    if (allocator == NULL) {
        return ERROR("allocator == NULL", ECODE(ALOCAS, FSA, ARGV));
    }

    if (allocator->page_list == NULL) {
        return ERROR("allocator has already been free", ECODE(ALOCAS, FSA, INIT));
    }
